...
function startMining() {
    ...
    miningInterval = setInterval(() => {
        const now = Date.now();
        const endTimeStr = localStorage.getItem('nexmine_mining_end');
        if (!endTimeStr) {
            stopMining();
            return;
        }
...
function addRewardToWallet(amount) {
    ...
    // Person A gets commission from Person B's mining
    ...
    if (referrer) {
        ...
        const commission = amount * 0.21; // Total simulated pool
...
function updateReferralUI() {
    ...
    const signupBonus = parseFloat(localStorage.getItem('nexmine_ref_signup_bonus') || '0.00');
    const totalEarnings = earnings + signupBonus;
    ...
    const totalPct = 0.21;
    const l1 = (earnings * (0.10 / totalPct)) + signupBonus; // Add signup bonus to level 1
