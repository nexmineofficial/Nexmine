...
function updateReferralUI() {
    ...
    const pcts = { l1: 0.10, l2: 0.05, l3: 0.04, l4: 0.03, l5: 0.01 };
    const signupBonusL1 = 100;

    let totalEarnings = 0;
    const levelEarnings = { l1: 0, l2: 0, l3: 0, l4: 0, l5: 0 };

    Object.keys(levelsData).forEach(lvl => {
        const members = levelsData[lvl];
        const pct = pcts[lvl];
        members.forEach(m => {
            const daysJoined = Math.max(1, Math.floor((Date.now() - (m.signupTime || Date.now())) / (1000 * 60 * 60 * 24)));
            const simulatedMined = daysJoined * 24 * 0.8; 
            let memberEarning = simulatedMined * pct;
            if (lvl === 'l1') memberEarning += signupBonusL1;
            levelEarnings[lvl] += memberEarning;
        });
        totalEarnings += levelEarnings[lvl];
    });
    ...
    const levels = [
        { id: 'ref-l1', val: levelEarnings.l1, countId: 'ref-l1-count', count: levelsData.l1.length },
        { id: 'ref-l2', val: levelEarnings.l2, countId: 'ref-l2-count', count: levelsData.l2.length },
        { id: 'ref-l3', val: levelEarnings.l3, countId: 'ref-l3-count', count: levelsData.l3.length },
        { id: 'ref-l4', val: levelEarnings.l4, countId: 'ref-l4-count', count: levelsData.l4.length },
        { id: 'ref-l5', val: levelEarnings.l5, countId: 'ref-l5-count', count: levelsData.l5.length }
    ];
